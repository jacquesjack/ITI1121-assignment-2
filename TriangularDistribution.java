...
public class TriangularDistribution {
	...
	public Rational pdf(int x) {
		...
		if ( x < a){
			...
			Rational one = new Rational(0,1);
			return one;

		}else if (a <= x && x < c){

			Rational two = new Rational(2*(x - a),(b - a)*(c - a));
			return two;

		}else if (x == c){

			Rational three = new Rational(2,b - a);
			return three;

		}else if (c < x && x <= b){

			Rational four = new Rational(2*(b - x),(b - a)*(b - c));
			return four;

		}else{
			Rational one1 = new Rational(0,1);
			return one1;
		}
